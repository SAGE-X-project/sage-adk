...
func main() {
	...
	chatbot, err := builder.NewAgent("anthropic-chatbot").
		...
		AfterStop(func(ctx context.Context) error {
			log.Println("Anthropic Chatbot Agent stopped")
			return nil
		}).
		Build()
...
// handleMessage creates a message handler that uses Anthropic Claude to generate responses
	...
	return func(ctx context.Context, msg agent.MessageContext) error {
		...
		request := &llm.CompletionRequest{
			Messages: []llm.Message{
				{Role: llm.RoleSystem, Content: "You are Claude, a helpful AI assistant created by Anthropic. Provide thoughtful and accurate responses."},
